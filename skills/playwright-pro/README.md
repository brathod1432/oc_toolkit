...
claude plugin install pw@ai-ops-skills
...
Slash commands that turn natural language into production-ready Playwright tests. Each skill leverages an agent runtime's built-in capabilities (`/batch` for parallel work, `Explore` for codebase analysis, `/debug` for trace inspection).
...
| Agent | How |
|---|---|
| **an agent runtime** | Full plugin â€” slash commands, MCP tools, hooks, agents |
| **an agent runtime** | Copy `AI_RUNTIME_GUIDE.md` to your project root as `AGENTS.md` |
...
- `/pw:generate` uses AI assistant's `Explore` subagent to understand your codebase before generating tests
