---
name: "promote"
description: "Graduate a proven pattern from auto-memory (MEMORY.md) to AI_RUNTIME_GUIDE.md or .claude/rules/ for permanent enforcement."
---

# /si:promote â€” Graduate Learnings to Rules

Moves a proven pattern from AI assistant's auto-memory into the project's rule system, where it becomes an enforced instruction rather than a background note.

## Usage

```
/si:promote <pattern description>                    # Auto-detect best target
/si:promote <pattern> --target claude.md             # Promote to AI_RUNTIME_GUIDE.md
...
- "What specific behavior should AI assistant follow?"
...
|---|---|---|
| Applies to entire project | `./AI_RUNTIME_GUIDE.md` | "Use pnpm, not npm" |
| Applies to specific file types | `.claude/rules/<topic>.md` | "API handlers need validation" |
| Applies to all your projects | `~/.claude/AI_RUNTIME_GUIDE.md` | "Prefer explicit error handling" |

If the user didn't specify a target, recommend one based on scope.

### Step 4: Distill into a concise rule

Transform the learning from auto-memory's note format into AI_RUNTIME_GUIDE.md's instruction format:
...
**After** (AI_RUNTIME_GUIDE.md â€” prescriptive):
```markdown
## Build & Dependencies
...
**For AI_RUNTIME_GUIDE.md:**
1. Read existing AI_RUNTIME_GUIDE.md
...
The pattern is now an enforced instruction. AI assistant will follow it in all future sessions.
```
...
- You corrected AI assistant about it more than once
...
### AI_RUNTIME_GUIDE.md vs .claude/rules/

| Use AI_RUNTIME_GUIDE.md for | Use .claude/rules/ for |
|---|---|
...
- Keep AI_RUNTIME_GUIDE.md under 200 lines â€” use rules/ for overflow
