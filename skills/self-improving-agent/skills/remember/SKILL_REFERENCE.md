...
Writes an explicit entry to auto-memory when something is important enough that you don't want to rely on AI assistant noticing it automatically.
...
| Project convention not in AI_RUNTIME_GUIDE.md | "We use barrel exports in src/components/" |
...
| Preference you want AI assistant to learn | "Don't add comments explaining obvious code" |
...
```
ðŸ’¡ This sounds like it could be a AI_RUNTIME_GUIDE.md rule rather than a memory entry.
...
âœ… Saved to auto-memory
  ...
  AI assistant will see this at the start of every session in this project.
```

## What NOT to use /si:remember for

- **Temporary context**: Use session memory or just tell AI assistant in conversation
- **Enforced rules**: Use `/si:promote` to write directly to AI_RUNTIME_GUIDE.md
- **Cross-project knowledge**: Use `~/.claude/AI_RUNTIME_GUIDE.md` for global rules
...
- If you're remembering the same thing twice, promote it to AI_RUNTIME_GUIDE.md
