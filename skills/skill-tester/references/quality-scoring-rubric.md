...
This document defines the comprehensive quality scoring methodology used to assess skills within the ai-ops-skills ecosystem. The scoring system evaluates four key dimensions by default (each weighted at 25%), with an optional fifth Security dimension (enabled via `--include-security` flag).
...
This dimension is critical for ensuring that skills do not introduce security risks into the ai-ops-skills ecosystem.
...
This quality scoring rubric ensures consistent, objective, and comprehensive assessment of all skills within the ai-ops-skills ecosystem while providing clear guidance for quality improvement.